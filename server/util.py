...
def get_cropped_image_if_2_eyes(image_path, image_base64_data):
    ...
    # A slightly more permissive face detector helps with phone images and varied angles.
    ...
    for (x,y,w,h) in faces:
            ...
            if len(eyes) < 2:
                eyes = eye_cascade_glasses.detectMultiScale(roi_gray)
            if len(eyes) >= 2:
                cropped_faces.append(roi_color)

    # Fallback: if eye detection fails, use the largest detected face.
    if len(cropped_faces) == 0 and len(faces) > 0:
